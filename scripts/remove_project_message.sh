...
if [ "${_PROJECT_PHASE_INSTALL_DRUPAL}" = "new" ]; then
	echo "(!!!) Removing drupal/core-project-message package." &&
		...
		php "${LANDO_MOUNT}/.project/tools/edit-json.php" "composer.json" "config ### allow-plugins ### drupal/core-project-message ### NULL" &&
		php "${LANDO_MOUNT}/.project/tools/edit-json.php" "composer.json" "extra ### drupal-core-project-message ### NULL" &&
